function REP = mopso_s(conf, params, MultiObj, mats, road_ts)
% MOPSO - Multi-Objective Particle Swarm Optimization
% Integrates with the Quarter-Car simulation workflow.
%
% Inputs:
%   conf:     Configuration struct (physics, road settings)
%   params:   Optimization parameters struct (Np, maxgen, W, C1, C2, etc.)
%   MultiObj: Problem definition struct (nVar, var_min, var_max)
%   mats:     System matrices (A, B, C, D)
%   road_ts:  Road profile timeseries
%
% Output:
%   REP:      Repository struct containing Pareto-optimal solutions
%             (REP.pos = gains, REP.pos_fit = objectives)
...
nVar    = MultiObj.nVar;        % Number of Variables (Kp, Ki, Kd)
...
% Initialize Fitness (Cost)
nObj = 2; % J1 (Comfort), J2 (Handling)
POS_fit = zeros(Np, nObj);

% Calculate Max Velocity Limit
MAX_VEL_LIMIT = (var_max - var_min) .* maxvel ./ 100;

disp('MOPSO: Initializing population...');

%% 3. Evaluate Initial Population
for i = 1:Np
    % Extract PID Gains for this particle
    Kp = POS(i, 1);
    Ki = POS(i, 2);
    Kd = POS(i, 3);
    ...
    try
        ...
        % Handle unstable particles (assign high cost)
        warning('Particle %d unstable: %s', i, ME.message);
        POS_fit(i, :) = [1e9, 1e9];
    end
end

%% 4. Initialize Repository & Personal Bests
PBEST     = POS;
PBEST_fit = POS_fit;
...
for gen = 1:maxgen
    ...
    pos_best = dominates(POS_fit, PBEST_fit);      % New dominates Old
    best_pos = ~dominates(PBEST_fit, POS_fit);     % Old does NOT dominate New

    % Randomly choose if neither dominates (diversity)
...
% Remove dominated solutions from the combined set
...
for dim = 1:ndim
    % Avoid crash if all solutions are identical (min == max)
...
for n = 1:npar
    ...
    % Create unique ID for hypercube
